...
{
	$sql = 	"SELECT * from bank ";//-1 trash, 0 inactive
	$category=$g->getSQLJSON($sql);
	return;
}
if($_POST['type']=='get_by_lat_lng')
{
	$sql = 	"SELECT latitude,longitude,atm_address,atm_name,
...
FROM atm_point WHERE bank_id=".$_POST["bank_id"]." HAVING distance <= 25" ;
