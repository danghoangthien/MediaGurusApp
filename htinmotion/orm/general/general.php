<?php   
  class general
  {
    ...
    function getSQL($sql,$mode=-1)
	  ...
	  function getSQLAdvance($sql,$mode=-1)
	    {	    	
	    	$db = ADONewConnection($this->databasetype); 
		    //$db->debug = true; 
			if(DEBUG) $db->debug = DEBUG; 
	  ...
	  function getSQLJSON($sql)
	  {
		  $rs=$this->getSQLAdvance($sql);
      ...
      function executeSQL($sql)
	    {	    	
		    ...
		    $rs = $db->Execute($sql);
			if ($rs === false) {
         		return 'error inserting: '.$db->ErrorMsg().'<BR>';
			}	
			else
			{
				return $rs;
	  ...
	  function getCacheSQLAdvance($sec,$sql,$mode=0)
	    {	    	
	    	$db = ADONewConnection($this->databasetype); 
		    if(DEBUG) $db->debug = DEBUG; //$db->debug = true; 
		    $db->Connect($this->server, $this->user, $this->password, $this->database); 
	  ...
	  function executeInsert($table,$array,$subSQL="")
	  {
		  ...
		  $valueList ="('".implode(array_map('mysql_real_escape_string',$array),"','")."')";
		  $pattern   ="INSERT INTO $table $columnList VALUES $valueList $subSQL";
		  $re=$this->executeSQL($pattern);
		  if( is_object( $re ) )
		  {
			  
			  $id= $this->getSQL("SELECT LAST_INSERT_ID() as last");
			  return $id[0]["last"];
		  }
		  else return $re;
	  }
	  ...
	  function executeUpdate($table,$array,$subSQL="")
	  {
		  ...
		  $db = ADONewConnection($this->databasetype); 
		  $db->Connect($this->server, $this->user, $this->password, $this->database); 
		  foreach ($array as $k=>$v) $pair[]=$k."='".mysql_real_escape_string($v)."'";
		  $pair=implode($pair,",");
		  $pattern="UPDATE $table SET $pair $subSQL";
		  $re=$this->executeSQL($pattern);
		  if( !is_object( $re ) ) {print_r($re);return $re; }
		  else return true;
	  }
